...
罫線、ブロック要素、ギリシャ文字、キリル文字などを半角とし、

※、♨、①、絵文字、NerdFontのプライベート領域などを全角としています。
...
## ローマ数字
- 和文では全角で扱われてきた歴史がある
- 欧文ではi, v, xの組み合わせで表現されることがあり、あまり利用されていない印象
...
## ○△□◇などの記号類
これらの記号類は大変難しい。
英語圏の人々はTUIでこれらのAmbiguous文字を半角で利用します。
一方JIS X 0208由来のこれら文字を和文で半角とすることは耐え難い。
幸いUnicodeには多くの記号類が収録されているので、
TUIアプリケーションの開発者に丁寧に説明し、明示的な文字幅の文字に置き換えてもらうことで問題を回避できる。
たとえば、以下のように代替できる。

```
[○] U+25CB → [◦] U+25E6
[●] U+25CF → [•] U+2022 もしくは [∙] U+2219
[△] U+25B3 → [▵] U+25B5
[▲] U+25B2 → [▴] U+25B4
[□] U+25A1 → [▫] U+25AB
[■] U+25A0 → [▪] U+25AA
...
`[•]` U+2022 BULLETがEAW=Aであるのが厄介だ、しかしこれはJIS X 0208に存在しなかった文字なので影響は軽微、このロケールでは半角とする。
これが受け入れられない場合、`[∙]` U+2219 BULLET OPERATOR(EAW=N)で代替すると良い。

TODO: 確実に全角となる記号を整理したいが、U+2BC5 BLACK MEDIUM UP-POINTING TRIANGLE CENTREDはEAW=Nのためこのロケールどうするか要検討
...
## ドミノ牌
- 縦の牌は半角に収まるが、横の牌は全角でないと難しい
- よく考えたらドミノ倒ししかやったことがなくゲームのルールを知らない

全角とする

## トランプのカード
- `🃏`のみWideであとのカードはNeutral
...
- いくつかの文字のみ半角で描画可能(``)
- NerdFontで提供されている`SymbolsNerdFontMono-Regular.ttf`ではこれらの文字も全角なので全て全角で扱うのが無難。
